...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    m_Controller.getTransStick_HAT_0().whileHeld(new SwervePivotAtPodBi(
        ...
        () -> m_Controller.getSpin(),
        0.0));
    m_Controller.getTransStick_HAT_45().whileTrue(new SwervePivotAtPodBi(
        () -> m_Controller.getForward(),
        () -> m_Controller.getStrafe(),
        () -> m_Controller.getSpin(),
        45.0));
    m_Controller.getTransStick_HAT_135().whileTrue(new SwervePivotAtPodBi(
        () -> m_Controller.getForward(),
        () -> m_Controller.getStrafe(),
        () -> m_Controller.getSpin(),
        135.0));
    m_Controller.getTransStick_HAT_180().whileTrue(new SwervePivotAtPodBi(
        () -> m_Controller.getForward(),
        () -> m_Controller.getStrafe(),
        () -> m_Controller.getSpin(),
        180.0));
    m_Controller.getTransStick_HAT_225().whileTrue(new SwervePivotAtPodBi(
        () -> m_Controller.getForward(),
        () -> m_Controller.getStrafe(),
        () -> m_Controller.getSpin(),
        225.0));
    m_Controller.getTransStick_HAT_315().whileTrue(new SwervePivotAtPodBi(
        () -> m_Controller.getForward(),
        () -> m_Controller.getStrafe(),
        () -> m_Controller.getSpin(),
        315.0));
    */
    ...
    // m_Controller.getRotStick_Button3().onFalse(new SwerveSpinLockOff());
    // m_Controller.getRotStick_Button3().onFalse(new SwerveSpinLockOff());
    ...
    m_Controller.getOp_A().whileTrue(new IntakingDirect2());
    m_Controller.getOp_A().onFalse(new DelayedIntakeStop());

    m_Controller.getOp_Y().whileTrue(new ShootSetVals());
    m_Controller.getOp_B().onTrue(new FlywheelStop());

    m_Controller.getOp_Back_FS().whileTrue(new IndexerBackWhenHeld());
    m_Controller.getOp_Start_FS().whileTrue(new IndexerForwardWhenHeld());
    m_Controller.getOp_Back_DS().onTrue(new ExtendIntake());
    m_Controller.getOp_Start_DS().onTrue(new RetractIntake());

    // m_Controller.getOp_A_DS().onTrue(new ClimbPistonEngage()); // TODO: CHECK IF TWO COMMANDS CAN BE MAPPED TO THE
    //                                                                // SAME BUTTON
    // m_Controller.getOp_A_DS().onTrue(new AnglerZeroAtMax());
    m_Controller.getOp_B_DS().onTrue(new ClimbPistonRetract());
    m_Controller.getOp_B_DS().onTrue(new AnglerZeroAtMax());
    // m_Controller.getOp_X_DS().onTrue(new ClimbPistonEngage());
    m_Controller.getOp_A_DS().onTrue(new ClimbPistonEngage());
    m_Controller.getOp_Y_DS().onTrue(new AnglerZeroAtMax());

    m_Controller.getOp_DPAD_UP().onTrue(new VisionDriverCam());
    m_Controller.getOp_DPAD_DOWN().onTrue(new VisionZoom2x());

    m_Controller.getOp_DPAD_LEFT().onTrue(new FlywheelAngleFender());
    m_Controller.getOp_DPAD_RIGHT().onTrue(new FlywheelAngleWall());

    m_Controller.getOp_A_FS().whileTrue(new AnglerZeroAtMax());
    m_Controller.getOp_Y_FS().onTrue(new FlywheelDefaultCommandStop());

    m_Controller.getOp_Back().whileTrue(new SpittingDown());
    m_Controller.getOp_Start().whileTrue(new SpittingUp());

    // m_Controller.getOp_Back().whileTrue(new FlywheelPIDToggleTest());
    // m_Controller.getOp_Start().whileTrue(new ShootPIDToggleTest());

    m_Controller.getOp_X().whileTrue(new FlywheelAngleVision());

    m_Controller.getOp_LeftTrigger().whileTrue(new ShootVision());

    m_Controller.getOp_RightTrigger().whileTrue(new FlywheelAngleVisionInt());
  }
