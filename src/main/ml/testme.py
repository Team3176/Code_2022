...
while(True):
    ...
    if len(pinkContours) > 0:
        largestPink = pinkContours[0]
        for contour in pinkContours:
            if(cv.contourArea(contour) > cv.contourArea(largestPink)):
                largestPink = contour
        x, y, w, h = cv.boundingRect(largestPink)
        outPink = cv.rectangle(outPink, (x,y), (x+w, y+h), (255,255,255), 1)
        pinkArea = cv.contourArea(largestPink)
        capture = pinkArea > 5000
    ...
    nt.putNumber('pink_area', pinkArea)
    nt.putBoolean('stop', stop)
    nt.putBoolean('capture', capture)
